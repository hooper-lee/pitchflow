...
- **云端数字员工**：Hemera Agent 支持租户级启用、套餐策略、Credits 记录、工作流权限、审批恢复执行、飞书/企微私聊绑定和后台监控
...
- 产品层策略：`Free / Pro / Business / Enterprise` Agent plan policy、Channel policy、Workflow policy
- Credits 与上下文：先校验团队是否启用 Agent，再做套餐 / 渠道 / credits 检查；上下文从服务端 `agent_messages` 按 `conversationId` 读取并按套餐裁剪，不依赖前端传入 messages
- 团队启用：`设置 -> 数字员工` 支持 team_admin 启用/停用 Hemera Agent、改名、生成飞书/企微绑定码
- Workflow 目标驱动：挖掘任务会从自然语言自动拆关键词；活动草稿和邮件策略草稿不再强行追问表单字段，执行前返回业务摘要
- 审批恢复执行：高风险工具进入 approval flow，team_admin 确认后恢复执行原工具并写入 audit / usage
- Channel 私聊：飞书 / 企业微信 webhook 签名校验、私聊绑定、external user 映射、Business+ 私聊进入 Agent runtime、主动文本回复
- Admin 监控：Agent 工具、套餐策略、运行记录、Tool Calls、Usage、审批记录、Channel Bindings、MCP Gateway 状态页；用户 / 租户列表展示本月 Agent Credits 用量和套餐额度

当前边界：

- 飞书/企微群聊只用于通知，不允许执行 Agent 写操作
- 敏感密钥仍必须走后台配置，不允许在聊天里收集
- MCP Gateway 和 Custom Skill 仍保留为 Enterprise 后续能力
...
- **用户管理** `(/admin/users)`：用户列表、角色、租户和本月 Agent Credits 用量
- **租户管理** `(/admin/tenants)`：租户套餐、API 配额、本月 Agent Credits 用量 / 套餐额度
- **Agent 工具** `(/admin/agent-tools)`：Agent 套餐策略、工具权限、风险等级、Credits，以及当前团队 Agent 启用 / 停用入口
- **Agent 运行** `(/admin/agent-runs)`：Agent Runs、Tool Calls、Usage 记录
- **Agent 审批** `(/admin/agent-approvals)`：高风险操作确认 / 拒绝
- **Agent Channel** `(/admin/agent-channels)`：飞书 / 企微外部用户绑定记录
...
- `设置 -> 数字员工`：启用/停用团队 Agent、修改名称、生成飞书/企微绑定码
- 右下角 Agent 浮窗：站内对话入口；未启用时普通成员会看到启用提示，team_admin 可直接启用
- 飞书私聊：团队管理员在 `设置 -> 数字员工` 填写飞书 App ID / App Secret，飞书后台事件订阅选择「长连接」并订阅「接收消息」事件；成员生成绑定码后，私聊机器人发送 `bind <绑定码>` 完成绑定
- 企微私聊：成员生成绑定码后，私聊机器人发送 `bind <绑定码>` 完成绑定；Business+ 套餐可通过私聊查询状态和推进允许的工作流
...
测试覆盖 Free 写操作拦截、Pro 创建挖掘任务、Business 飞书/企微渠道、未启用 Agent 提示、usage credits 基础记录和目标驱动 workflow。
